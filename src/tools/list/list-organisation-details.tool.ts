...
const ListOrganisationDetailsTool = CreateXeroTool(
  ...
  async () => {
    ...
    const resolvedExternalLinks =
      organisation.externalLinks?.map(
        (link, index) =>
          `${index + 1}. ${link.linkType}: ${link.url ? getExternalLink(link.url) : link.url}`,
      ) || [];
    ...
    const paymentTerms = organisation.paymentTerms
      ? Object.entries(organisation.paymentTerms)
          .map(([key, value], index) => {
    ...
    const organisationDetails = [
      ...
      organisation.organisationType
        ? `Organisation Type: ${organisation.organisationType}`
        : null,
      ...
      organisation.registrationNumber
        ? `Registration Number: ${organisation.registrationNumber}`
        : null,
      organisation.taxNumber ? `Tax Number: ${organisation.taxNumber}` : null,
      organisation.organisationEntityType
        ? `Organisation Entity Type: ${organisation.organisationEntityType}`
        : null,
      ...
      organisation.periodLockDate
        ? `Period Lock Date: ${organisation.periodLockDate}`
        : null,
      organisation.organisationStatus
        ? `Organisation Status: ${organisation.organisationStatus}`
        : null,
      ...
      organisation.lineOfBusiness
        ? `Line of Business: ${organisation.lineOfBusiness}`
        : null,
      `Addresses:\n${addresses}`,
      `Phone Numbers:\n${phones}`,
