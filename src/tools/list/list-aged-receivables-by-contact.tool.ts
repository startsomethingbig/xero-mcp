...
const ListAgedReceivablesByContact = CreateXeroTool(
  ...
  This shows aged receivables for a certain contact up to a report date.`,
  {
    ...
    reportDate: z
      ...
      .optional()
      .describe(
        "Optional date to retrieve aged receivables in YYYY-MM-DD format. If none is provided, defaults to end of the current month.",
      ),
    invoicesFromDate: z
      .string()
      .optional()
  ...
  async ({ contactId, reportDate, invoicesFromDate, invoicesToDate }) => {
    const response = await listXeroAgedReceivablesByContact(
      contactId,
      reportDate,
      invoicesFromDate,
      invoicesToDate,
